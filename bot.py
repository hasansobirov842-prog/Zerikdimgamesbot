...
# MUHIM:
# 100 ta noyob odam /start qilgandan keyin
# majburiy homiylik avtomatik o'chadi.
SPONSOR_LIMIT = 100
...
def init_db():
    ...
    # Standart homiylar mavjud bo'lmasa qo'shamiz.
    for channel, url in DEFAULT_SPONSORS:
        conn.execute(
            """
...
def disable_sponsors_if_limit():
    ...
    total = int(row["total_users"] or 0) if row else 0

    if total >= SPONSOR_LIMIT:
        conn.execute(
            """
...
def add_user(user_id, username):
    ...
    # 100 ga yetishi bilan avtomatik o'chiramiz.
    if is_new and int(total) >= SPONSOR_LIMIT:
        disable_sponsors_if_limit()
...
async def show_sponsor_required(update):
    text = (
        "🔒 <b>Botdan foydalanish uchun homiyga obuna bo‘ling.</b>\n\n"
        "1️⃣ Quyidagi kanallarga obuna bo‘ling.\n"
        "2️⃣ Keyin <b>✅ TASDIQLASH</b> tugmasini bosing.\n\n"
        f"👥 Botga kirganlar: <b>{get_total_users()}/{SPONSOR_LIMIT}</b>"
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 100 ta odam to'lganda homiylik o'chadi.
    if total >= SPONSOR_LIMIT:
        disable_sponsors_if_limit()
...
async def stars_get(update, context):
    text = (
        "⭐ <b>STARS OLISH</b>\n\n"
        "Bu bo‘limda topshiriqlar va boshqa usullar "
        "orqali Stars ishlashingiz mumkin.\n\n"
        f"💰 Balansingiz: <b>{get_points(update.effective_user.id):.2f} ⭐</b>"
    )
...
async def stars_work(update, context):
    await update.message.reply_text(
        ...
        f"Har bir o‘yin g‘alabasi: <b>+{GAME_REWARD} ⭐</b>",
        parse_mode="HTML",
    )
...
def can_play(user_id):
    context_key = f"cooldown_{user_id}"
...
async def start_game(update, context, game_type):
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton(
                        "20",
                        callback_data="logic:20",
                    ),
                    InlineKeyboardButton(
                        "24",
                        callback_data="logic:24",
                    ),
                    InlineKeyboardButton(
                        "32",
                        callback_data="logic:32",
                    ),
                    InlineKeyboardButton(
                        "36",
                        callback_data="logic:36",
                    ),
                ]
            ]),
        )
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton(
                        "BOT",
                        callback_data="word:BOT",
                    ),
                    InlineKeyboardButton(
                        "BAT",
                        callback_data="word:BAT",
                    ),
                    InlineKeyboardButton(
                        "BIT",
                        callback_data="word:BIT",
                    ),
                ]
            ]),
        )
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton(
                        "739",
                        callback_data="code:739",
                    ),
                    InlineKeyboardButton(
                        "427",
                        callback_data="code:427",
                    ),
                    InlineKeyboardButton(
                        "915",
                        callback_data="code:915",
                    ),
                ]
            ]),
        )
    ...
    if game_type == "speed":
        ...
        await query.edit_message_text(
            "⚡ <b>TEZLIK</b>\n\n"
            "GO tugmasini imkon qadar tez bosing!",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton(
...
async def withdraw(update, context):
    ...
    if pending:
        await update.message.reply_text(
            "⏳ Sizda allaqachon <b>pending</b> yechish so‘rovi bor.\n\n"
...
async def process_withdraw_amount(update, context):
    ...
    # Pulni bir marta yechib qo'yamiz.
    conn.execute(
        """
        UPDATE users
        SET points=points-?
    ...
    try:
        ...
        logger.error("Admin xabari xatosi: %s", e)
...
async def buy_stars(update, context):
    await update.message.reply_text(
        ...
        "Telegram Stars sotib olish uchun quyidagi tugmani bosing.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [
                InlineKeyboardButton(
...
async def callbacks(update, context):
    ...
    if data == "check_sponsors":
        if not sponsor_active():
            await query.edit_message_text(
                "✅ <b>Homiylik majburiyati o‘chirildi.</b>\n\n"
                "Botdan foydalanishingiz mumkin.",
                parse_mode="HTML",
            )
        ...
        if not ok:
            await query.answer(
                "❌ Hali barcha homiylarga obuna bo‘lmagansiz!",
                show_alert=True,
            )
            return
...
def admin_keyboard():
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton(
                "🔴/🟢 HOMIYLIK",
                callback_data="admin_toggle_sponsor",
            ),
        ],
    ])
...
async def admin_callback(update, context):
    ...
    if data == "admin_stats":
        ...
        await query.edit_message_text(
            ...
            f"📢 Homiylik: "
            f"<b>{'YOQILGAN' if sponsor_active() else 'O‘CHIRILGAN'}</b>\n"
    ...
    if data == "admin_sponsors":
        ...
        text += (
            f"Holat: "
            f"<b>{'ON' if sponsor_active() else 'OFF'}</b>\n"
            f"100 ta odamga yetganda avtomatik OFF."
        )
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [
                    InlineKeyboardButton(
                        "➖ HOMIY O‘CHIRISH",
                        callback_data="admin_remove_sponsor",
                    )
                ],
                [
                    InlineKeyboardButton(
    ...
    if data == "admin_toggle_sponsor":
        ...
        new_state = 0 if row and row["active"] else 1

        conn.execute(
            """
            UPDATE sponsor_settings
            SET active=?, disabled_at=?
            WHERE id=1
            """,
            (
                new_state,
                None if new_state else now_iso(),
            ),
        )
        ...
        await query.answer(
            "Homiylik " +
            ("yoqildi." if new_state else "o‘chirildi."),
            show_alert=True,
        )
    ...
    if data == "admin_add_sponsor":
        context.user_data["admin_add_sponsor"] = True
    ...
    if data == "admin_remove_sponsor":
        context.user_data["admin_remove_sponsor"] = True
...
async def admin_text_handler(update, context):
    ...
    if context.user_data.get("admin_add_sponsor"):
        ...
        await update.message.reply_text(
            f"✅ Homiy qo‘shildi: {channel}"
        )
        return

    if context.user_data.get("admin_remove_sponsor"):
        context.user_data.pop(
...
async def reject_withdraw(update, context, withdrawal_id):
    ...
    # Rad etilsa pulini balansga qaytaramiz.
    conn.execute(
        """
        UPDATE users
        SET points=points+?
    ...
    await query.edit_message_text(
        ...
        f"🆔 User: <code>{row['user_id']}</code>\n"
        f"⭐ Miqdor: <b>{row['amount']}</b>\n\n"
        "Balans foydalanuvchiga qaytarildi.",
        parse_mode="HTML",
    )
...
async def text_handler(update, context):
    ...
    # Avval admin rejimlarini tekshiramiz.
    if update.effective_user.id == ADMIN_ID:
        if any(
    ...
    # Homiylik hali yoqilgan bo'lsa tekshiramiz.
    if sponsor_active():
        ok = await check_all_sponsors(
            context.bot,
            update.effective_user.id,
