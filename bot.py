...
# Stars sotib olish uchun kanal posti
STARS_BUY_URL = "https://t.me/premyumstarstekin/933"

# Stars yechish shartlari
MIN_WITHDRAW = 50
MIN_REFERRALS = 10
...
def main_menu():
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("💸 STARS YECHISH", callback_data="withdraw"),
            InlineKeyboardButton("⭐ STARS OLISH", callback_data="buy_stars")
        ],
        [
...
async def quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    question, answers, correct = random.choice(QUIZZES)
...
async def island(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i in range(0, 4, 2):
        keyboard.append([
            InlineKeyboardButton(
                answers[i],
                callback_data=f"text:{answers[i]}"
            ),
            InlineKeyboardButton(
                answers[i + 1],
                callback_data=f"text:{answers[i+1]}"
            )
        ])
...
async def daily(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row and row[0] == today:
        text = (
            "⏳ <b>Daily bonusni bugun olding.</b>\n\n"
            "Ertaga yana olasan!"
        )
...
async def balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row = con.execute(
        """
        SELECT points,wins,games,referrals
        FROM users
        WHERE id=?
        """,
        (q.from_user.id,)
...
async def referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if refs >= 10:
        status = "🔓 10 ta do‘st talabi bajarildi!"

    else:
        status = (
            f"🔒 Yana {10 - refs} ta do‘st kerak."
        )

    await q.edit_message_text(
        "👥 <b>DO‘ST TAKLIF QILISH</b>\n\n"
        "Har bir yangi do‘st uchun ⭐ +3\n\n"
        f"👥 Sizning referral: {refs}/10\n"
        f"{status}\n\n"
        f"🔗 <code>{link}</code>",
        reply_markup=InlineKeyboardMarkup([
            [
...
async def buy_stars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        "⭐ <b>STARS OLISH</b>\n\n"
        "Stars narxlari va sotib olish uchun "
        "quyidagi kanal postiga o‘ting 👇\n\n"
        "📢 Narxlarni kanaldan ko‘rishingiz mumkin.",
...
async def withdraw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row = con.execute(
        """
        SELECT points,referrals,username
        FROM users
        WHERE id=?
        """,
        (q.from_user.id,)
    ...
    if referrals < MIN_REFERRALS:
        await q.edit_message_text(
            "💸 <b>STARS YECHISH</b>\n\n"
            f"🔒 Kamida {MIN_REFERRALS} ta do‘st taklif qilish kerak.\n\n"
            f"👥 Hozir: {referrals}/{MIN_REFERRALS}\n"
            f"👥 Yana: {MIN_REFERRALS - referrals} ta\n\n"
            "Do‘stlaringni taklif qil va shartni bajar! 🚀",
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton(
                        "👥 DO‘ST TAKLIF",
    ...
    # Minimal balans sharti
    if points < MIN_WITHDRAW:
        await q.edit_message_text(
            "💸 <b>STARS YECHISH</b>\n\n"
            f"❌ Minimal yechish: ⭐ {MIN_WITHDRAW}\n\n"
            f"⭐ Sizning balansingiz: {points}\n"
            f"⭐ Yetishmayapti: {MIN_WITHDRAW - points}\n\n"
            "O‘yin o‘ynab yana ball yig‘ing! 🎮",
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton(
                        "🎮 O‘YINLAR",
    ...
    await q.edit_message_text(
        "💸 <b>STARS YECHISH</b>\n\n"
        f"⭐ Balansingiz: {points}\n"
        f"👥 Referral: {referrals}/10\n\n"
        f"Minimal yechish: ⭐ {MIN_WITHDRAW}\n\n"
        "Yechib olish uchun qancha ⭐ kerakligini "
        "yozing.\n\n"
        f"Masalan: <code>{MIN_WITHDRAW}</code>",
        reply_markup=InlineKeyboardMarkup([
            [
                InlineKeyboardButton(
                    "❌ BEKOR QILISH",
...
async def withdraw_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.user_data.get("waiting_withdraw_amount"):
        return
    ...
    if referrals < MIN_REFERRALS:
        ...
        await update.message.reply_text(
            f"❌ Sizda hali {MIN_REFERRALS} ta referral yo‘q."
        )
        return
    ...
    if amount > points:
        ...
        await update.message.reply_text(
            f"❌ Balansingizda faqat ⭐ {points} bor."
        )
        return

    # So‘rovni bazaga saqlaymiz
    now = datetime.now(timezone.utc).isoformat()
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        ...
        f"⭐ Jami ball: {points}\n"
        f"💸 Kutilayotgan yechishlar: {pending}",
        reply_markup=admin_menu(),
        parse_mode="HTML"
    )
